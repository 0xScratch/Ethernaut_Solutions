/*
    ...
    Anyways, let's begin and just nail our last 'Ethernaut' challenge!!
*/

// First, we need to look at the code, which you must have probably been..Our task is to turn the switch on, and it can be done by calling the "turnSwitchOn" function, but wait there's a modifier which is stopping us from doing that, and what this modifier says -> you can call this function just with the help of this contract...means 'internal' spooky thing going on here

// Then, you have noticed that 'flipSwitch' function which takes a calldata and can only call some functions, but yea yea..if you look carefully and quite try to understand the code (if you feel difficulty in that, come on just use that damn chatGPT). After getting the code understood, you will find that flipSwitch only allows us to call the 'turnSwitchOff' function which is not really helpful for us
...
// Congratulations!
// Must pat your back cuz you completed this challenge and followed it to the end..